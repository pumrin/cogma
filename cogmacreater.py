...
# 1小節ごとのコード進行の指定　デフォルト4→2に変更
# parser.add_argument('--chordbeat', type=int, default=4)
parser.add_argument('--chordbeat', type=int, default=2)
# 生成される曲の小節数　デフォルト4→8に変更
...
# 揺らぎ設定(a,b)の時、aはコード進行がオートのとき使用されて、bはコード進行が指定されているときに使用される
# parser.add_argument('--top_p', type=str, default='45,7')
parser.add_argument('--top_p', type=str, default='45,80')
...
# wavファイルがinputされていないときはc2mが自動でコード進行を作成し、chordsに入れる
# wavファイルがinputされているときは、コード推定を行い、その結果をchordsに入れる
if args.input == None:
    chords = [c for c in args.chord.split('|') if len(c)>0]
else:
    chords = [c for c in  bs.beatstest(args.input).split('|') if len(c)>0]
...
def main():
    cur_top = (0,top_p[1])
    # beatstest(wav_read(filename))

    with tf.compat.v1.Session(graph=tf.Graph()) as sess:
        context = tf.compat.v1.placeholder(tf.int32, [1, None])
        output = model.model(hparams=hparams, X=context)
        vars = [v for v in tf.compat.v1.trainable_variables() if 'model' in v.name]

        saver = tf.compat.v1.train.Saver(var_list=vars)
        ckpt = tf.compat.v1.train.latest_checkpoint(args.model)
